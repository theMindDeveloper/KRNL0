...
describe('frame add', () => {
  ...
  it('--near centers frame on source node and seeds childIds', async () => {
    // source-node: position={x:100, y:200}, w=200, h=80
    // srcCenter = (100 + 100, 200 + 40) = (200, 240)
    // frame top-left = (200 - 360/2, 240 - 240/2) = (20, 120)
    const res = await frameAdd(ctx, { near: 'source-node' });
    expect(res.ok).toBe(true);
    const frames = getFrames();
    expect(frames[0]!.position!.x).toBe(20);
    expect(frames[0]!.position!.y).toBe(120);
